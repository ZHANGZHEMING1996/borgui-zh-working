...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // Navigation sections with headings - moved inside component to access t function
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const navigationSections: any[] = [
    {
      segment: 'dashboard',
      items: [
        { name: t('navigation.dashboard'), href: '/dashboard', icon: Home, key: 'dashboard' as const },
        { name: t('navigation.activity'), href: '/activity', icon: History, key: 'dashboard' as const },
      ],
    },
    {
      heading: t('navigation.sections.backup_section'),
      items: [
        {
          name: t('navigation.remote_machines'),
          href: '/ssh-connections',
          icon: Computer,
          key: 'connections' as const,
        },
        { name: t('navigation.repositories'), href: '/repositories', icon: Database, key: 'repositories' as const },
        { name: t('navigation.backup'), href: '/backup', icon: FileText, key: 'backups' as const },
        { name: t('navigation.archival'), href: '/archives', icon: Archive, key: 'archives' as const },
        { name: t('navigation.restore'), href: '/restore', icon: Download, key: 'restore' as const },
        { name: t('navigation.schedule'), href: '/schedule', icon: Clock, key: 'schedule' as const },
      ],
    },
    {
      heading: t('navigation.sections.settings_section'),
      items: [
        {
          name: t('navigation.settings.personal'),
          icon: User,
          key: 'dashboard' as const,
          subItems: [
            { name: t('navigation.settings.account'), href: '/settings/account', icon: User },
            { name: t('navigation.settings.appearance'), href: '/settings/appearance', icon: Palette },
            { name: t('navigation.settings.notifications'), href: '/settings/notifications', icon: Bell },
            { name: t('navigation.settings.preferences'), href: '/settings/preferences', icon: Sliders },
          ],
        },
        {
          name: t('navigation.settings.system'),
          icon: SettingsIcon,
          key: 'dashboard' as const,
          subItems: [
            { name: t('navigation.settings.system_settings'), href: '/settings/system', icon: SettingsIcon },
            { name: t('navigation.settings.cache'), href: '/settings/cache', icon: Server },
            { name: t('navigation.settings.logs'), href: '/settings/logs', icon: FileText },
            { name: t('navigation.settings.packages'), href: '/settings/packages', icon: Package },
          ],
        },
        {
          name: t('navigation.settings.management'),
          icon: HardDrive,
          key: 'dashboard' as const,
          subItems: [
            { name: t('navigation.settings.mounts'), href: '/settings/mounts', icon: HardDrive },
            { name: t('navigation.settings.scripts'), href: '/settings/scripts', icon: FileCode },
            { name: t('navigation.settings.users'), href: '/settings/users', icon: Users },
            { name: t('navigation.settings.export_import'), href: '/settings/export', icon: DownloadIcon },
          ],
        },
        {
          name: t('navigation.settings.advanced'),
          icon: Zap,
          key: 'dashboard' as const,
          subItems: [{ name: t('navigation.settings.beta'), href: '/settings/beta', icon: Zap }],
        },
      ],
    },
  ]
  ...
  const drawer = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ mt: 'auto', p: 2, borderTop: 1, borderColor: 'divider' }}>
        <Tooltip title={t('navigation.sidebar.version_info')} arrow placement="right">
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            <Info size={16} style={{ marginRight: 8, color: '#666' }} />
            <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600 }}>
              {t('navigation.sidebar.version_info')}
            </Typography>
          </Box>
        </Tooltip>
        {systemInfo ? (
          <Box sx={{ ml: 3 }}>
            <Typography
              ...
              sx={{ lineHeight: 1.5 }}
            >
              {t('navigation.sidebar.app_version')}: {systemInfo.app_version}
            </Typography>
